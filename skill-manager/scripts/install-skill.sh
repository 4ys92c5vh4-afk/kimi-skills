...
# GitHub API token support (higher rate limits for API calls)
GH_API_HEADERS=()
if [[ -n "${GITHUB_TOKEN:-}" ]]; then
    GH_API_HEADERS=(-H "Authorization: Bearer $GITHUB_TOKEN")
elif [[ -n "${GH_TOKEN:-}" ]]; then
    GH_API_HEADERS=(-H "Authorization: Bearer $GH_TOKEN")
fi
...
if $IS_ROOT_REPO; then
  ...
  if [[ -d "$SKILL_DIR" ]]; then
    ...
    git -c http.lowSpeedLimit=1000 -c http.lowSpeedTime=20 fetch origin "$BRANCH"
  ...
  download_dir() {
    ...
    local response=$(curl -sL --connect-timeout 10 --max-time 30 "${GH_API_HEADERS[@]}" "https://api.github.com/repos/$OWNER/$REPO/contents/$api_path?ref=$BRANCH")
    ...
    echo "$response" | jq -c '.[]' 2>/dev/null | while read -r item; do
      ...
      if [[ "$type" == "file" ]]; then
        ...
        curl -sL --connect-timeout 10 --max-time 90 "$download_url" -o "$local_dir/$name"
      elif [[ "$type" == "dir" ]]; then
